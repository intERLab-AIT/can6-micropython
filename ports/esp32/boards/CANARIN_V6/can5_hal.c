...
static can5_err_t cell_start(bool enable_gps, const char *apn, bool enable_sntp, const char *sntp_server);
...
static struct hal_hdl_s {
    ...
    .uport = {
        ...
        .config.u ={
#if CONFIG_IDF_TARGET_ESP32 || HACK_IO_BOARD
...
#endif
#if HACK_IO_BOARD //this should be properly handled as PM_ENABLE_PIN
    gpio_config_t io_conf;
    //disable interrupt
    io_conf.intr_type  = GPIO_INTR_DISABLE;
    //set as output mode
    io_conf.mode = GPIO_MODE_OUTPUT;
    io_conf.pin_bit_mask = BIT64(__hhal.uport.config.u.en); // | BIT64(__hhal.uport.config.u.le) | BIT64(__hhal.adport.config.ad.le);
...
static can5_err_t cell_start(bool enable_gps, const char *apn, bool enable_sntp, const char *sntp_server)
{
    ...
    ESP_LOGI(TAG, "Cell Mode enable_gps: %s enable_sntp: %s", boolean_get_str(enable_gps), boolean_get_str(enable_sntp));
...
static uint64_t __wifi_get_next_connect()
{
    ...
    return __hhal.wifi.connect_retries * 2 * 1000000;
}
